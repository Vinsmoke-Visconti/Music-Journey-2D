...
export class Vehicle {
  ...
  private _setDimensions(): void {
    switch (this.vehicleType) {
      ...
      case 'pickup':
        ...
        break;
      default: // van
        this.W = 140; this.H = 60; this.WR = 20;
        this.wFrontX =  this.W / 2 - 28;
        this.wRearX  = -this.W / 2 + 28;
    }
  }
  ...
  private _drawVan(): void {
    ...
    g.beginFill(VAN_COLORS.roof); g.drawRoundedRect(-W/2+10,-H-28,W-30,30,4); g.endFill();
    g.beginFill(VAN_COLORS.stripe,0.9); g.drawRect(-W/2,-H*0.45,W,5); g.endFill();

    // Windshield
    g.beginFill(VAN_COLORS.windowFrame); g.drawRoundedRect(W/2-42,-H-25,35,27,3); g.endFill();
    g.beginFill(VAN_COLORS.window,0.85); g.drawRoundedRect(W/2-40,-H-23,31,23,2); g.endFill();
    g.beginFill(0xffffff,0.15); g.drawRect(W/2-39,-H-22,8,10); g.endFill();

    // Side windows
    for (let i=0;i<2;i++){
      const wx=-W/2+10+i*40;
      g.beginFill(VAN_COLORS.windowFrame); g.drawRoundedRect(wx,-H-25,32,27,3); g.endFill();
      g.beginFill(VAN_COLORS.window,0.8);  g.drawRoundedRect(wx+2,-H-23,28,23,2); g.endFill();
      g.beginFill(0xffffff,0.12); g.drawRect(wx+3,-H-22,7,9); g.endFill();
    }
    // Lights
    g.beginFill(VAN_COLORS.light); g.drawRoundedRect(W/2-8,-H+10,10,8,2); g.endFill();
    g.beginFill(0xffffff,0.5); g.drawRoundedRect(W/2-7,-H+11,6,5,1); g.endFill();
    g.beginFill(0xff4444); g.drawRoundedRect(-W/2-2,-H+10,8,8,2); g.endFill();
    // Bumpers
    g.beginFill(VAN_COLORS.bumper);
    g.drawRoundedRect(W/2-6,-H+20,10,14,2);
    g.drawRoundedRect(-W/2-4,-H+20,10,14,2);
    g.endFill();
  ...
  private _updateWheelPositions(): void {
    const R = this.WR;
    // Position wheels using their X offsets and rotate them independently
  ...
  private _updateWheelRotation(): void {
    this.wheelAngle+=this.speed*0.12;
  }
  ...
  private _applyTransforms(_t: number): void {
    ...
    // Suspension relative to the road
    this.suspensionContainer.y = this.suspY - this.WR * 1.8;
